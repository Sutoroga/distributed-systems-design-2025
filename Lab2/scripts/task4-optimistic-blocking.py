...
def update_worker(client_id):
    ...
    logger.info(f"[Client-{client_id}] Starting 10K increments with optimistic locking")

    for _ in range(10000):
        while True:
            old_value = my_map.get(key)
            new_value = {"amount": old_value["amount"] + 1}
...
def main():
    ...
    my_map = client.get_map("race-map").blocking()
    my_map.put("optimistic-key", {"amount": 0})
    client.shutdown()
    ...
    result = client.get_map("race-map").blocking().get("optimistic-key")["amount"]
